...
|---|---|---|
| `*.test.*`, `*.spec.*`, `__tests__/` | `the rune-test rule` | Test patterns, assertions, no `test.skip`, coverage rules |
| `migrations/`, `schema.*`, `*.prisma` | `the rune-db rule` | Migration safety, rollback script, parameterized queries |
| `Dockerfile`, `*.yml` (CI/CD), `terraform/` | `the rune-deploy rule` | Deployment checklist, no hardcoded secrets |
| `docs/*.md`, `README.md`, `CHANGELOG.md` | `the rune-docs rule` | Documentation patterns, no stale references |
| `SKILL.md`, `PACK.md` | `the rune-skill-forge rule` | Skill template compliance, frontmatter validation |
| `.env*`, `*secret*`, `*credential*` | `the rune-sentinel rule` | Security scan mandatory, never commit secrets |
