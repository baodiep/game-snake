...
- If source needs changes → hand off to `the rune-fix rule`. Test's job ends at the test file.
...
10. MUST NOT modify source/implementation files — test writes test files ONLY, hand off source changes to the rune-fix rule
...
| Modifying source files to make tests work | HIGH | Role boundary: test writes test files ONLY — source changes go to the rune-fix rule |
