...
All four layers are necessary. During testing, each layer catches bugs the others miss — different code paths bypass single validation points. When recommending a fix via `the rune-fix rule`, explicitly call out which layers need validation added.
...
- `the rune-fix rule` will preserve these markers until the bug is fully resolved and tests pass
